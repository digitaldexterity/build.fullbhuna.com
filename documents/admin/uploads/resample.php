...
<main>
  <div class="container clearfix">
    ...
    <!-- InstanceBeginEditable name="Body" -->
      <h1><i class="glyphicon glyphicon-folder-open"></i> Resampling all Images</h1>
      <script>
	  var i = 0;
	  $(document).ready ( 
    ...
    function resampleImage() {
		i++;
        var tester = $.ajax({
                url: "resample.ajax.php?filename="+filename[i],
				async: false,
	  ...
	  var filecount = 0;
        <?php do { ?>
		filecount ++;
          filename[filecount] = '<?php echo htmlentities($row_rsImports['newfilename']); ?>';
		   document.write("<div id='result"+filecount+"'></div>" );
		   
		   
          <?php } while ($row_rsImports = mysql_fetch_assoc($rsImports)); ?>
