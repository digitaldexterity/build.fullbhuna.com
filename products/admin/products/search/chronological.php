...
mysql_select_db($database_aquiescedb, $aquiescedb);
$query_rsProductSearch = sprintf("SELECT productsearch.createddatetime, productsearch.searchterm, productsearch.sessionID, track_session.username FROM productsearch LEFT JOIN track_session ON (productsearch.sessionID = track_session.ID) WHERE DATE(productsearch.createddatetime) >= %s AND DATE(productsearch.createddatetime) <= %s AND (productsearch.regionID = %s ) GROUP BY productsearch.searchterm ORDER BY createddatetime DESC ", GetSQLValueString($varStartDate_rsProductSearch, "date"),GetSQLValueString($varEndDate_rsProductSearch, "date"),GetSQLValueString($varRegionID_rsProductSearch, "int"));
...
if(isset($_GET['csv'])) {
	...
	exportCSV(array("Date|date","Search Term","Session|hide","User"), $rsProductSearch, $filename="Product-Search-YY-MM-DD");
	exit;
}
...
<div class="page class">
      ...
      <nav class="navbar navbar-default navbar-expand-lg navbar-light bg-light"><div class="container-fluid"><ul class="nav navbar-nav">
        <li class="nav-item"><a href="../index.php" class="nav-link"><i class="glyphicon glyphicon-arrow-left"></i> Products</a></li>
        <li class="nav-item"><a href="index.php" class="nav-link"><i class="glyphicon glyphicon-arrow-left"></i> Search Count</a></li>
  ...
  <p class="text-muted">Search terms<?php echo ($startRow_rsProductSearch + 1) ?> to <?php echo min($startRow_rsProductSearch + $maxRows_rsProductSearch, $totalRows_rsProductSearch) ?> of <?php echo $totalRows_rsProductSearch ?> <a href="<?php $url =  $_SERVER['REQUEST_URI']; $url .= strpos($url,"?")>0 ? "&" : "?"; $url .= "csv=true"; echo $url; ?>" class="link_csv" >Download as spreadsheet</a></p>
        ...
        <thead>
          <tr>
            ...
            <th>Search term</th><th>User</th><th>View</th>           </tr></thead><tbody>
          <?php do { ?>
            <tr><td><?php echo date('d M Y H:i', strtotime($row_rsProductSearch['createddatetime'])); ?></td>
              <td><?php echo $row_rsProductSearch['searchterm']; ?></td><td><?php echo $row_rsProductSearch['username']; ?></td>
        <td><a href="../../../../core/seo/admin/visitors/visitor-session.php?sessionID=<?php echo $row_rsProductSearch['sessionID']; ?>" class="link_view">View</a></td>      
            </tr>
