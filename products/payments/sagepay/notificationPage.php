...
$strSQL = "SELECT SecurityKey FROM productorders where VendorTxCode='" . mysql_real_escape_string($strVendorTxCode) . "' and VPSTxId='" . mysql_real_escape_string($strVPSTxId) . "'";
	...
	** if you need clarification on this process **/
	...
	if ($strMySignature!==$strVPSSignature)
	{
		...
		** send back a Status of INVALID and failure page RedirectURL **/
		...
		$strSQL = "UPDATE productorders set Status='TAMPER WARNING! Signatures do not match for this Order.  The Order was Cancelled. strMySignature=" . $strMySignature . " strVPSSignature=" . $strVPSSignature ."' 
	...
	{     
		...
		$strSQL="UPDATE productorders set Status='" . $strDBStatus . "'";
		
		if(@$strStatus=="OK") $strSQL=$strSQL . ",AmountPaid=Amount";
		...
		$select = "SELECT successURL, failURL FROM productprefs WHERE ID = ".$regionID;
		...
		if ($strStatus=="OK" || $strStatus=="AUTHENTICATED" || $strStatus=="REGISTERED") {
			...
			confirmationEmail($strVendorTxCode,"COMPLETED");
			$purchasemade = 2; // paid transaction
			...
			$strRedirectPage= (isset($prefs['successURL']) && trim($prefs['successURL']) !="") ? $prefs['successURL']."?VendorTxCode=" . $strVendorTxCode : $strYourSiteFQDN . $strVirtualDir ."/orderSuccessful.php?VendorTxCode=" . $strVendorTxCode;
		} else
			/** The status indicates a failure of one state or another, so send the customer to orderFailed instead **/
			$strRedirectPage= (isset($prefs['failURL']) && trim($prefs['failURL']) !="")  ? $prefs['failURL']."?VendorTxCode=" . $strVendorTxCode :  $strYourSiteFQDN . $strVirtualDir ."/orderFailed.php?VendorTxCode=" . $strVendorTxCode;
