...
$query_rsPhotos = sprintf("SELECT photos.*, photocategories.categoryname, photocategories.soundtrackURL FROM photos LEFT JOIN photocategories ON (photocategories.ID = photos.categoryID) WHERE (%s  = 0 OR categoryID = %s) AND photos.active = 1", GetSQLValueString($varGalleryID_rsPhotos, "int"),GetSQLValueString($varGalleryID_rsPhotos, "int"));
...
<title>Fullscreen Gallery: <?php echo $row_rsPhotos['categoryname'] ; ?></title>
...
@-webkit-keyframes fadey{0%{opacity:0;}15%{opacity:1;}85%{opacity:1;}100%{opacity:0;}}
...
#slideshow-container:fullscreen { display: flex; justify-content: center; align-items: center;}
#slideshow-container:fullscreen figure { width: 80%; margin: 0 auto; }
:-webkit-fullscreen { width: 100%; height: 100%; }
*:-moz-fullscreen { background: #fff; }
...
<body><a href=# onclick=" fullScreen(document.getElementById('slideshow')) ">Full Screen</a> | <a href="/photos/gallery/index.php?galleryID=<?php echo isset($_GET['galleryID']) ? intval($_GET['galleryID']) : 0; ?>">Back</a>
