<?php require_once('../../Connections/aquiescedb.php'); ?><?php require_once('../../core/includes/adminAccess.inc.php'); ?><?php require_once('../../core/includes/autolinks.inc.php'); ?><?php require_once('../includes/galleryfunctions.inc.php'); ?>
<?php
if (!isset($_SESSION)) {
  session_start();
}
...
$query_rsPhotoCategory = sprintf("SELECT * FROM photocategories WHERE active !=2 AND regionID = %s ORDER BY categoryname ASC", GetSQLValueString($varRegionID_rsPhotoCategory, "int"));
...
if(isset($_POST['createddatetime'])) { // post
	if($_POST['categoryID'] == -1) { // new category
		$insert = "INSERT INTO photocategories (accesslevel, active, addedbyID, categoryname, createddatetime) VALUES (0,1,".GetSQLValueString($_POST['postedbyID'], "int").",".GetSQLValueString($_POST['newcategory'], "text").", NOW())";
	...
	if(isset($_POST['photo']) && ($row_rsMediaPrefs['uploadpermissioncheck'] == 0 || $_POST['permission']==1)) {
		...
		header("location: index.php?galleryID=".$galleryID); exit;
	}
...
<main>
  ...
  <div id="pagePhotoReview" style="position:relative; z-index:5000;" class="container pageBody">
    <h1><i class="glyphicon glyphicon-picture"></i> Review your pictures</h1>
	...
	if(count($files)>2) { 
      ...
      <div class="photoItem">
        ...
        <div class="photoDetails">
          ...
          <input  class="form-control" name="title[<?php echo $key; ?>]" type="text" id="title<?php echo $key; ?>" value="<?php echo substr($value,11); // remove random prefix ?>" size="50" maxlength="50" />
          
          <label class="photoDescription" for="description[<?php echo $key; ?>]">Description:                </label>
          
          <textarea name="description[<?php echo $key; ?>]" id="description<?php echo $key; ?>" cols="50" rows="5" class="form-control"></textarea>
  ...
  if($count>0) { ?>
      ...
      <p  class="form-inline">Add selected photos to: 
        <span id="spryselect1">
        <label>
...
?>
      ...
      <?php } else { ?><p>
      <label class="form-inline">Initial status: <select name="statusID" id="statusID" class="form-control">
                    <?php
do {  
?>
...
?>
 ...
 />
      ...
      <p>
        ...
        <?php } else { ?>
        <p class="alert warning alert-warning" role="alert">No images are available to add to a gallery. <a href="index.php">Please try again</a>.<?php if($_SESSION['MM_UserGroup']==10) { ?><br><br>WEBADMIN: <?php echo $path; } ?></p>  
        <?php } ?>
      </form>
