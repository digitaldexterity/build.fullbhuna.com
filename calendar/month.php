...
$date = intval($_GET['year'])."-".str_pad(intval($_GET['month']),2,"0",STR_PAD_LEFT)."-01";
...
$varDate_rsEvents = "1970-01-01";
if (isset($date)) {
  $varDate_rsEvents = $date;
}
mysql_select_db($database_aquiescedb, $aquiescedb);
$query_rsEvents = sprintf("SELECT event.ID, eventgroup.eventtitle, eventcategory.title AS eventcategory, location.locationname, event.startdatetime FROM event LEFT JOIN eventgroup ON (event.eventgroupID = eventgroup.ID) LEFT JOIN eventcategory ON (eventgroup.categoryID = eventcategory.ID) LEFT JOIN location ON (event.eventlocationID = location.ID) WHERE (eventgroup.categoryID = %s OR %s = 0) AND DATE(event.startdatetime) >= %s AND DATE(event.startdatetime ) < (%s + INTERVAL 1 MONTH) AND event.statusID = 1 AND eventgroup.usertypeID <= %s GROUP BY eventgroupID ORDER BY eventgroup.featured DESC, event.startdatetime ", GetSQLValueString($varCategoryID_rsEvents, "int"),GetSQLValueString($varCategoryID_rsEvents, "int"),GetSQLValueString($varDate_rsEvents, "date"),GetSQLValueString($varDate_rsEvents, "date"),GetSQLValueString($varUserGroup_rsEvents, "int"));
...
<title><?php $pageTitle = "Month's Events"; echo $pageTitle." | ".$site_name; ?></title>
<!-- InstanceEndEditable -->
...
$month = (isset($_GET['month']) && $_GET['month']!="") ? intval($_GET['month']) : $d["mon"]; 
$year = (isset($_GET['year']) && $_GET['year'] !="") ? intval($_GET['year']) : $d["year"];

?> 
<div class="crumbs"><div>You are in: <a href="/">Home</a><span class="separator">&nbsp;&rsaquo;&nbsp;</span><a href="/calendar/index.php">Calendar</a><span class="separator">&nbsp;&rsaquo;&nbsp;</span>Month's Events<?php if(isset($row_rsThisCategory['title'])) {  echo ": ".$row_rsThisCategory['title']; } ?></div></div>
  <div id="monthCalendarContainer"><?php echo $cal->getMonthView($month, $year);
?>
   
  </div>
  <h1>Month's Events<?php echo (isset($row_rsThisCategory['title'])) ? ": ".$row_rsThisCategory['title'] : ""; ?></h1>
  <?php if ($totalRows_rsCategories > 0) { // Show if recordset not empty ?>
  <form action="month.php" method="get" name="form1" target="_top" id="form1" role="form">
    <label>
...
<?php if (isset($row_rsThisCategory['description']) && $row_rsThisCategory['description'] !="") { ?><p><?php echo nl2br($row_rsThisCategory['description']); ?></p>
  ...
  <div class="eventList"><?php if ($totalRows_rsEvents == 0) { // Show if recordset empty ?>
...
<?php if ($totalRows_rsEvents > 0) { // Show if recordset not empty ?>
  <ul>
    <?php do { ?>
      <li>
        <div  class="datetime"><span class="eventdate"><time itemprop="startDate" datetime="<?php echo date('Y-m-dTH:i:s',strtotime($row_rsEvents['startdatetime'])); ?>"><?php echo date('D d M Y',strtotime($row_rsEvents['startdatetime'])); ?></time></span> <span class="eventtime"><?php echo date('g:ia',strtotime($row_rsEvents['startdatetime'])); ?></span> <span class="eventduration"><?php echo isset($row_rsEvents['enddatetime']) ?  showDuration($row_rsEvents['startdatetime'], $row_rsEvents['enddatetime']) : ""; ?></span></div>
        <div><span class="eventtitle"><a href="/calendar/event.php?eventID=<?php echo $row_rsEvents['ID']; ?>"><?php echo $row_rsEvents['eventtitle']; ?></a></span> <span class="eventlocation"><?php echo isset($row_rsEvents['locationname']) ? $row_rsEvents['locationname'] : ""; ?></span> <span class="eventcategory"><?php echo $row_rsEvents['eventcategory']; ?></span></div>
        
      </li>
      <?php } while ($row_rsEvents = mysql_fetch_assoc($rsEvents)); ?>
  </ul>
  <?php } // Show if recordset not empty ?></div>
   <p><a href="index.php" >Coming up</a></p>
   <p><a href="search.php" class="link_back"><i class="glyphicon glyphicon-arrow-left"></i> Search events</a></p></div>
    <!-- InstanceEndEditable -->
    </main>
    <!-- ISEARCH_END_INDEX --> 
