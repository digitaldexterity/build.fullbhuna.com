...
function exportCSV($headers, $recordset, $filename="Export-YY-MM-DD", $delimiter=",",$excelcompat=true,$addquotes=true,$linefeed="\r\n") {
	...
	// if filename is a PATH then SAVE
	
	global $database_aquiescedb, $aquiescedb;
	mysql_select_db($database_aquiescedb, $aquiescedb);
	
	$csv = "";
	if((is_object($recordset) || is_resource($recordset)) && mysql_num_rows($recordset)>0) { // is data
		$headerformats = array();
		...
		if(is_array($headers)) { // is headers
			...
			while($header = array_shift($headers)) { // loop
				...
				if($format[1]!="hide") {
					$csv .= ($col > 0 ? $delimiter : ""); 
					$csv .= (formatCSV($format[0],"text",$delimiter,$excelcompat,$addquotes)); $col++; 
				} // not hide
			} // end loop
			$csv .= ($linefeed);
		} // end is headers
		...
		while($row = mysql_fetch_assoc($recordset)) {
			...
			foreach($row as $key => $value) {
				if(@$headerformats[$col] !="hide") {					
						$csv .= (($item > 0) ? $delimiter : ""); 
						$item++;				
						$csv .= (formatCSV($value,@$headerformats[$col],$delimiter,$excelcompat,$addquotes));						
					}
				$col ++;
				} // end foreach
			$csv .= ($linefeed);
		}
		if(strpos($filename,"/")===false) { //download
			csvHeaders($filename,$delimiter);
			print($csv);
			die();
		} else { // save
			saveFile(SITE_ROOT.$filename,$csv);
...
function showMedia($mediaURL, $size="", $video_attr = "controls", $force_link= false) {
	...
	if(!$force_link && preg_match("/[\w\-]+\.(jpg|png|gif|jpeg|mp4)/", $mediaURL)) {
		...
		echo "<a href=\"".$mediaURL."\" class=\"btn btn-default btn-secondary\"><i class=\"glyphicon glyphicon-download-alt\"></i> Download file</a>";
	}
	} else {
...
function createURLname($urlname="", $title="", $replace="-", $table = "", $id = 0) {
	global $database_aquiescedb, $aquiescedb, $regionID;
	...
	$urlname = preg_replace("/[^a-zA-Z0-9_\-]/", $replace, $urlname);
	$urlname = preg_replace('/'.$replace.'+/', $replace, $urlname);
...
function deleteFile($file) {
	if(strpos($file,SITE_ROOT)===false) {
		$file = SITE_ROOT.$file;
	}
