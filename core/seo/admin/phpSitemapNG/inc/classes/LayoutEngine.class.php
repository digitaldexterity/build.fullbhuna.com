...
*/    	$this->content['content_header'] = "";
    ...
    function addWarning($msg, $title = "") {
    	if ($title != "") {
    		$tmp = '<h4 class="alert warning alert-warning" role="alert">Warning: '.$title.'</h4>'."\n".'<div class="alert warning alert-warning" role="alert">'.$msg.'</div>'."\n";
    	} else {
    		$tmp = '<div class="alert warning alert-warning" role="alert">Warning: '.$msg.'</div>'."\n";
    	}
    	if ($this->buffering) {
			$this->content['body'] = array();
    		$this->content['body'][] = $tmp;
